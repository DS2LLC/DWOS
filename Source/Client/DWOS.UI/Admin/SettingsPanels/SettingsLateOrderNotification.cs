...
{
    public partial class SettingsLateOrderNotification
        : UserControl, ISettingsPanel
    {
        #region Fields

        public const string TEMPLATE_ID = "LateOrderNotification";
        public const string TEMPLATE_TOKENS = "%ORDER%, %PURCHASEORDER%, %SERIALNUMBERS%, %PART%, %ORIGINALSHIPDATE%, %ADJUSTEDSHIPDATE%";
        ...
        public SettingsLateOrderNotification()
        {
            InitializeComponent();
        }
        ...
        public void SaveData()
        {
            using (var ta = new TemplatesTableAdapter())
            {
                ...
                if (template == null)
                {
                    ...
                    template.Description = "Email template to notify customer that an order will be late.";
